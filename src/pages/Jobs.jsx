...
export default function Jobs({ preset }) {
  const { jobs } = useJobs(), applicationOnly = preset === 'applications'
  ...
  return <>
    {!applicationOnly && <div className="freshness-grid"><article><span>New in 24h</span><strong>{counts.day}</strong></article><article><span>New in 7 days</span><strong>{counts.week}</strong></article><article><span>80%+ Matches</span><strong>{counts.high}</strong></article><article><span>Remote Jobs</span><strong>{counts.remote}</strong></article><article><span>Total Indexed</span><strong>{counts.total}</strong></article></div>}
    ...
    <div className="page-heading"><div><p>{displayed.length} {applicationOnly ? 'applications in your pipeline' : 'jobs in the indexed dataset'}</p></div>{!applicationOnly && <button className="button button-primary" onClick={() => setModal(true)}><Plus size={18}/> Add Job Manually</button>}</div>
